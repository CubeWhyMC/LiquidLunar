...
public class ModelBendsZombie extends ModelBiped
{
    ...
    public void setRotationAngles(float argSwingTime, float argSwingAmount, float argArmSway, float argHeadY, float argHeadX, float argNr6, Entity argEntity)
    {
    	...
    	ZombieData data = (ZombieData) ZombieData.get(argEntity.getEntityId());
    	...
    	this.renderRotation.set(data.renderRotation);
    		...
    		if(ZombieData.get(argEntity.getEntityId()).canBeUpdated()){
    			...
    			BendsVar.tempData = ZombieData.get(argEntity.getEntityId());
    			
    			if(ZombieData.get(argEntity.getEntityId()).motion.x == 0.0f & ZombieData.get(argEntity.getEntityId()).motion.z == 0.0f){
    				AnimatedEntity.getByEntity(argEntity).get("stand").animate((EntityLivingBase)argEntity, this, ZombieData.get(argEntity.getEntityId()));
					BendsPack.animate(this,"zombie","stand");
				}else{
					AnimatedEntity.getByEntity(argEntity).get("walk").animate((EntityLivingBase)argEntity, this, ZombieData.get(argEntity.getEntityId()));
	        ...
	        ZombieData.get(argEntity.getEntityId()).syncModelInfo(this);
    }
	...
	public void updateWithEntityData(AbstractClientPlayer argPlayer){
		ZombieData data = ZombieData.get(argPlayer.getEntityId());
		if(data != null){
			this.renderOffset.set(data.renderOffset);
			this.renderRotation.set(data.renderRotation);
		}
